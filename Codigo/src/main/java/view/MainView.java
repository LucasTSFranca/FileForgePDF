...
public class MainView extends JFrame {
    ...
    public MainView() {
        ...
        this.fileConvertRepository = new FileConvertRepository();
        initUI();
        loadHistory();
    }
    ...
    private void initUI() {
        ...
        // ── Painel inferior (histórico do banco) ───────────────────
    ...
    private JPanel createHistoryPanel() {
        JPanel panel = createSectionPanel("Historico de Conversoes");
        ...
        JButton btnRefresh = createStyledButton("Atualizar", LARANJA);
        btnRefresh.addActionListener(e -> loadHistory());

        JButton btnDelete = createStyledButton("Excluir Selecionado", VERMELHO);
        btnDelete.addActionListener(e -> deleteSelectedHistory());
        btnDelete.addActionListener(f -> FileRepository.limparFile());

        JButton btnOpenHist = createStyledButton("Abrir PDF", LARANJA);
        btnOpenHist.addActionListener(e -> openHistoryPdf());

        toolbar.add(btnRefresh);
        toolbar.add(btnDelete);
        toolbar.add(btnOpenHist);

        panel.add(toolbar, BorderLayout.NORTH);

        // Tabela
        String[] columns = { "ID", "Nome", "Caminho", "Tamanho (KB)", "Data" };
    ...
    private JPanel createStatusBar() {
        ...
        statusLabel = new JLabel("Pronto. Adicione arquivos .txt para converter.");
            ...
            protected Void doInBackground() {
                if (merge) {
                    try {
                        ...
                        publish(result);
                        success = 1;
                        SwingUtilities.invokeLater(() -> progressBar.setValue(1));
                    } catch (Exception e) {
                        fail = 1;
                        System.err.println("Erro ao mesclar arquivos: " + e.getMessage());
                        SwingUtilities.invokeLater(() -> setStatus("Erro ao mesclar: " + e.getMessage(), VERMELHO));
                    }
                } else {
    ...
    private void loadHistory() {
        SwingWorker<List<FileConvertModel>, Void> worker = new SwingWorker<>() {
            @Override
            protected List<FileConvertModel> doInBackground() {
                try {
                    return fileConvertRepository.showAll();
                } catch (Exception e) {
            ...
            protected void done() {
                try {
                    List<FileConvertModel> list = get();
                    historyTableModel.setRowCount(0);
                    if (list != null) {
                        java.time.format.DateTimeFormatter formatter = java.time.format.DateTimeFormatter
                                .ofPattern("dd/MM/yyyy");
                        for (FileConvertModel f : list) {
                            String dataStr = f.getDt_criacao() != null ? f.getDt_criacao().format(formatter) : "-";
                            historyTableModel.addRow(new Object[] {
                                    f.getId(),
                                    f.getNome(),
                                    f.getNovoCaminho(),
                                    String.format("%.2f", f.getTamanho()),
                                    dataStr
                            });
                        }
                        setStatus("Histórico carregado — " + list.size() + " registro(s)", TEXT_PRIMARY);
                    } else {
                        setStatus("Não foi possível carregar o histórico do banco.", VERMELHO);
                    }
                } catch (Exception e) {
                    setStatus("Erro ao carregar histórico: " + e.getMessage(), VERMELHO);
                }
            }
        };
        worker.execute();
    ...
    private void deleteSelectedHistory() {
        ...
        if (row < 0) {
            setStatus("Selecione um registro do histórico para excluir.", LARANJA);
            return;
        }
        Long id = (Long) historyTableModel.getValueAt(row, 0);
        String nome = (String) historyTableModel.getValueAt(row, 1);

        int confirm = JOptionPane.showConfirmDialog(
                this,
                "Excluir registro '" + nome + "' do banco de dados?",
                "Confirmar Exclusão",
                JOptionPane.YES_NO_OPTION,
                JOptionPane.WARNING_MESSAGE);
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                fileConvertRepository.delete(id);
                historyTableModel.removeRow(row);
                setStatus("Registro '" + nome + "' excluído do banco.", TEXT_PRIMARY);
            } catch (Exception e) {
                setStatus("Erro ao excluir: " + e.getMessage(), VERMELHO);
            }
        }
    }

    private void openHistoryPdf() {
        int row = historyTable.getSelectedRow();
        if (row < 0) {
            setStatus("Selecione um registro do histórico para abrir.", LARANJA);
            return;
        }
        String path = (String) historyTableModel.getValueAt(row, 2);
        try {
            ...
            if (pdfFile.exists()) {
                ...
                setStatus("Arquivo não encontrado: " + path, VERMELHO);
            }
        } catch (Exception e) {
            setStatus("Erro ao abrir arquivo: " + e.getMessage(), VERMELHO);
        }
